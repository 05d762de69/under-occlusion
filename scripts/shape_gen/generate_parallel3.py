    ...
    *,
    ...
    while i <= end_i:
        ...
        while attempts < max_attempts_per_image and (not valid):
            ...
            # If repair produced multiple polygons, keep the largest one
            if poly.geom_type == "MultiPolygon":
                parts = [g for g in poly.geoms if (not g.is_empty) and g.area > 1e-6]
                if not parts:
                    fail_counts["final_poly_invalid"] += 1
                    continue
                poly = max(parts, key=lambda g: g.area)

            if poly.geom_type != "Polygon" or (not poly.is_valid) or poly.area <= 1e-6:
                fail_counts["final_poly_invalid"] += 1
                continue
